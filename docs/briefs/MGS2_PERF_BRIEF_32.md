...
**ОШИБКА, исправлено в #33.** Здесь было написано, что `render 45` это десятичное
45 = 1+4+8+32, то есть `EGL_OPENGL_BIT` присутствует. Wine печатает это поле через
`%x` (`win32u/opengl.c:497`), значит `45` это **шестнадцатеричное**:
...
То есть 32-битный Mali desktop GL не объявлял, и вывод «поэтому production
работает» был неверен. Настоящая причина — в `MGS2_RG353VS_HANDOFF.md`: развёрнутый
`win32u_glfuncs3.so` несёт правки, которых **нет в дереве исходников**, включая
приём ES2/ES3-битов и `EGL_CONTEXT_MAJOR_VERSION 3` по умолчанию. Handoff прямо
предупреждает, что пересборка их потеряет, и что пересобранный бинарник на 47 740
байт меньше. Подробности и результат — в брифе #33.

...
# Patch-14 diagnostic guard: exact-zero CC7/CC11 are restored before a note.
# This remained enabled in a production run that still lost SFX on an enemy
# transition, so it is NOT a fix.  Do not broaden the threshold: first capture
# dmime curves and controller changes on the effective note channel.
export MGS2_DMSYNTH_UNMUTE_NOTES="${MGS2_DMSYNTH_UNMUTE_NOTES:-1}"

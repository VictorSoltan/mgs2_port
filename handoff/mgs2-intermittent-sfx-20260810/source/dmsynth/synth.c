...
/*
 * MGS2's downloaded DLS effects are rendered around 40 dB below the regular
 * DirectSound mix on the RK3566 route.  Keep Wine's default behaviour unless
 * the port explicitly requests a gain correction.  The value is whole dB and
 * limited to a conservative range to avoid a malformed environment value
 * turning into clipped full-scale output.
 */
static int mgs2_dmsynth_gain_boost(void)
{
    char value[16];
    DWORD len = GetEnvironmentVariableA("MGS2_DMSYNTH_BOOST_DB", value, sizeof(value));
    int i = 0, gain = 0;

    if (!len || len >= sizeof(value)) return 0;
    while (value[i] >= '0' && value[i] <= '9')
    {
        gain = gain * 10 + value[i] - '0';
        if (gain >= 30) return 30;
        ++i;
    }
    return gain;
}
...
/*
 * Cost policy for FluidSynth on this port.
 *
 * The whole synthesiser is emulated x86 under box86, and it runs on the audio
 * thread of every port the game opens, every WRITE_PERIOD, whether or not a
 * note is sounding: FluidSynth's reverb and chorus are processed per block
 * regardless of the active voice count (fluid_rvoice_mixer_process_fx).  That
 * is why enabling DirectMusic routing made the console lag while silence was
 * free.  Each knob below removes per-sample work, not features the game asks
 * for by name.
 *
 * MGS2_DMSYNTH_FX=1         restore reverb/chorus (default: off)
...
int mgs2_env_int(const char *name, int def, int lo, int hi)
{
    ...
    if (!len || len >= sizeof(value)) return def;
...
static BOOL mgs2_dmsynth_fx_enabled(void)
{
    static int enabled = -1;

    if (enabled == -1) enabled = mgs2_env_int("MGS2_DMSYNTH_FX", 0, 0, 1);
    return enabled;
}
...
struct synth
{
    ...
    /* Last channel attenuation actually pushed into FluidSynth. */
    double applied_attenuation;
    BOOL volumes_applied;

    /* One-shot diagnostics for the MGS2 SFX path.  Disabled unless the
     * launcher explicitly sets MGS2_SHOTPROBE=1. */
    BOOL shotprobe_pending;
    DWORD shotprobe_group;
};

/* DirectMusic addresses MIDI channels as (channel group, channel).  The
 * shared-port MGS2 layout keeps groups 1 and 2 in one FluidSynth instance;
 * preserve that group component instead of collapsing both groups onto the
 * low MIDI nibble.  Group zero is retained as the legacy fallback used by
 * callers that do not fill dwChannelGroup. */
/* Spreading channel groups over separate FluidSynth channels is correct
 * DirectMusic behaviour, but it moves MGS2's group-2 notes off the channels its
 * DLS instruments were downloaded to, and that combination was measured silent.
 * Keep the collapsed Wine behaviour unless the mapping is asked for. */
static BOOL mgs2_dmsynth_groupmap(void)
{
    static int enabled = -1;

    if (enabled == -1) enabled = mgs2_env_int("MGS2_DMSYNTH_GROUPMAP", 0, 0, 1);
    return enabled;
}

static unsigned int synth_event_channel(const struct synth *This, const struct event *event)
{
    DWORD group = event->channel_group;

    if (mgs2_dmsynth_groupmap() && group >= 1 && group <= This->params.dwChannelGroups)
        return (group - 1) * 16 + (event->midi[0] & 0x0f);
...
static BOOL shotprobe_enabled(void)
{
    ...
    len = GetEnvironmentVariableA("MGS2_SHOTPROBE", value, sizeof(value));
    enabled = len && len < sizeof(value) && value[0] != '0';
    return enabled;
}

static unsigned int synth_voice_count(struct synth *This)
{
    struct voice *voice;
    unsigned int count = 0;

    LIST_FOR_EACH_ENTRY(voice, &This->voices, struct voice, entry)
        if (voice->fluid_voice && fluid_voice_is_playing(voice->fluid_voice)) ++count;
    return count;
}

/*
 * Cheapest available saving on this device: MGS2 runs fourteen synths, and each
 * one's sink calls Render fifty times a second whether or not a note is
 * sounding.  FluidSynth's block pipeline runs regardless.  With reverb and
 * chorus off (the default here) a block with no playing voice is exactly
 * silence, so write the silence instead of synthesising it.  Costs one walk of
 * the voice list per block, which is nothing next to the pipeline it skips.
 *
 * MGS2_DMSYNTH_IDLE_SKIP=0 restores unconditional synthesis.
 */
static BOOL mgs2_dmsynth_idle_skip(void)
{
    static int enabled = -1;

    if (enabled == -1) enabled = mgs2_env_int("MGS2_DMSYNTH_IDLE_SKIP", 1, 0, 1);
    return enabled;
}

static void synth_write_block(struct synth *This, short *buffer, DWORD frames)
{
    if (mgs2_dmsynth_idle_skip() && !mgs2_dmsynth_fx_enabled() && !synth_voice_count(This))
    {
        memset(buffer, 0, frames * 2 * sizeof(short));
        return;
    }

    fluid_synth_write_s16(This->fluid_synth, frames, buffer, 0, 2, buffer, 1, 2);
}

static unsigned int synth_download_count(struct list *list)
{
    struct list *entry;
    unsigned int count = 0;

    LIST_FOR_EACH(entry, list) ++count;
    return count;
}
...
/* DirectMusic addresses MIDI channels inside a channel group.  FluidSynth
 * exposes the same layout as one flat channel array, so keep group 1 on
 * channels 0..15 and place group 2 on 16..31 instead of collapsing both onto
 * channel 0..15. */
static unsigned int synth_channel_count(const struct synth *This)
{
    DWORD groups = This->params.dwChannelGroups;

    if (!groups) groups = 1;
    return groups * 16;
}

/* synth_Render reapplied the attenuation of every channel on every render
 * period.  With multiple channel groups that is 32 or more emulated
 * fluid_synth_set_gen() calls per sink per period, for a value that changes
 * only when the game moves a volume.  Track the applied value instead. */
static void update_channel_volumes(struct synth *This)
{
    double attenuation = (This->volume0 + This->volume1) * -0.1;
    unsigned int chan;

    if (This->volumes_applied && attenuation == This->applied_attenuation) return;

    for (chan = 0; chan < synth_channel_count(This); chan++)
        update_channel_volume(This, chan);

    This->applied_attenuation = attenuation;
...
static void synth_reset_default_values(struct synth *This, DWORD reason)
{
    ...
    for (chan = 0; chan < synth_channel_count(This); chan++)
    {
        ...
        fluid_synth_cc(This->fluid_synth, chan | 0xb0 /* CONTROL_CHANGE */, 0x5b /* EFFECTS_DEPTH1 / Reverb Send */,
                mgs2_dmsynth_fx_enabled() ? 40 : 0);
...
static HRESULT WINAPI synth_Open(IDirectMusicSynth8 *iface, DMUS_PORTPARAMS *params)
{
    ...
    /* Wine's old implementation silently left FluidSynth at its default
     * sixteen channels even when DirectMusic opened multiple groups. */
    fluid_settings_setint(This->fluid_settings, "synth.midi-channels",
            actual.dwChannelGroups * 16);
    /* Reverb and chorus are processed for every rendered block even with no
     * voices sounding, on every port, in emulated code.  Off unless asked. */
    fluid_settings_setint(This->fluid_settings, "synth.reverb.active",
            mgs2_dmsynth_fx_enabled() && !!(actual.dwEffectFlags & DMUS_EFFECT_REVERB));
    ...
    gain += mgs2_dmsynth_gain_boost() * 10.;
        ...
        DWORD groups)
{
    ...
    if (!groups || groups > This->caps.dwMaxChannelGroups)
        return E_INVALIDARG;
    if (This->open)
        return DMUS_E_SYNTHACTIVE;
    This->params.dwChannelGroups = groups;
...
static HRESULT WINAPI synth_Download(IDirectMusicSynth8 *iface, HANDLE *ret_handle, void *data, BOOL *ret_free)
{
    ...
    *ret_free = TRUE;
    ...
    if (info->dwDLType == DMUS_DOWNLOADINFO_INSTRUMENT || info->dwDLType == DMUS_DOWNLOADINFO_INSTRUMENT2)
        patch = ((DMUS_INSTRUMENT *)((BYTE *)data + offsets[0]))->ulPatch;

    switch (info->dwDLType)
    {
    case DMUS_DOWNLOADINFO_INSTRUMENT:
    case DMUS_DOWNLOADINFO_INSTRUMENT2:
        hr = synth_download_instrument(This, info, offsets, data, ret_handle);
        break;
    case DMUS_DOWNLOADINFO_WAVE:
        *ret_free = FALSE;
        hr = synth_download_wave(This, info, offsets, data, ret_handle);
        break;
    ...
    if (shotprobe_enabled() && (info->dwDLType == DMUS_DOWNLOADINFO_INSTRUMENT ||
            info->dwDLType == DMUS_DOWNLOADINFO_INSTRUMENT2 || info->dwDLType == DMUS_DOWNLOADINFO_WAVE))
    {
        unsigned int instruments, waves;
        EnterCriticalSection(&This->cs);
        instruments = synth_download_count(&This->instruments);
        waves = synth_download_count(&This->waves);
        LeaveCriticalSection(&This->cs);
        ERR("SHOTPROBE download synth=%p type=%lu id=%lu patch=%#x result=%#lx instruments=%u waves=%u\n",
                This, info->dwDLType, info->dwDLId, patch, hr, instruments, waves);
    }
        ...
        REFERENCE_TIME time, BYTE *buffer, DWORD size)
{
    ...
    while ((data = (BYTE *)(head + 1)) < end)
    {
        ...
        else
        {
            ...
            event->channel_group = head->dwChannelGroup;

            /* G4 of the SE chain: what actually crossed into dmsynth.  Budget
             * bounded so it cannot turn into per-event logging. */
            if (mgs2_env_int("MGS2_SECHAIN", 0, 0, 1))
            {
                static LONG g4_budget;

                if (InterlockedIncrement(&g4_budget) <= 400)
                    ERR("MGS2 SECHAIN G4-playbuffer tick=%lu synth=%p group=%lu cbEvent=%lu flags=%#lx midi=%02x %02x %02x\n",
                            GetTickCount(), This, event->channel_group, head->cbEvent,
                            head->dwFlags, event->midi[0], event->midi[1], event->midi[2]);
            }
            synth_sink_probe_mark(This->sink, This, event->channel_group,
                    event->midi[0] & 0xf0, event->midi[1], event->midi[2]);

            if (shotprobe_enabled() && event->midi[0] == MIDI_NOTE_ON &&
                    event->midi[1] == 0x3c && event->midi[2] == 0x7f)
                ERR("SHOTPROBE queued synth=%p group=%lu channel=%u note=%u velocity=%u time=%I64d sample=%I64d\n",
        ...
        DWORD length, LONGLONG position)
{
    ...
    struct voice *voice;
    short *render_start = buffer;
    DWORD render_length = length;
    BOOL shot_rendered = FALSE;

    TRACE("(%p, %p, %ld, %I64d)\n", This, buffer, length, position);

    EnterCriticalSection(&This->cs);
    update_channel_volumes(This);
    LIST_FOR_EACH_ENTRY_SAFE(event, next, &This->events, struct event, entry)
    {
        BYTE status = event->midi[0] & 0xf0;
        unsigned int chan = synth_event_channel(This, event);
        LONGLONG offset = event->position - position;
        if (offset >= length) break;
        if (offset > 0)
        {
            synth_write_block(This, buffer, offset);
            buffer += offset * 2;
            position += offset;
            length -= offset;
        }
        ...
        case MIDI_NOTE_ON:
        {
             ...
             * were invisible to it.  Bounded by count, not by rate. */
            if (mgs2_env_int("MGS2_NOTEPROBE", 0, 0, 1) && event->midi[2] > 0)
            {
                static LONG noteprobe_count;

                if (InterlockedIncrement(&noteprobe_count) <= 120)
                {
                    int sf = -1, bk = -1, pg = -1;
                    fluid_synth_get_program(This->fluid_synth, chan, &sf, &bk, &pg);
                    ERR("MGS2 NOTEPROBE synth=%p group=%lu chan=%u note=%u vel=%u sfont=%d bank=%d program=%d voices=%d\n",
                            This, event->channel_group, chan, event->midi[1], event->midi[2],
            ...
            if (shot)
            {
                shot_rendered = TRUE;
                fluid_synth_get_program(This->fluid_synth, chan, &sfont_id, &bank, &program);
                voices_before = synth_voice_count(This);
                This->shotprobe_pending = TRUE;
            ...
            if (mgs2_env_int("MGS2_NOTEPROBE", 0, 0, 1) && event->midi[2] > 0)
            {
                ...
                if (InterlockedIncrement(&after_count) <= 120)
                    ERR("MGS2 NOTEPROBE after synth=%p noteon=%d voices=%d\n",
                            This, noteon_result, synth_voice_count(This));
            }

            if (shot)
            {
                This->shotprobe_pending = FALSE;
                voices_after = synth_voice_count(This);
                ERR("SHOTPROBE render synth=%p group=%lu channel=%u sfont=%d bank=%d program=%d note=%u velocity=%u noteon=%d voices=%d->%d sample=%I64d\n",
                        This, event->channel_group, chan, sfont_id, bank, program,
                        event->midi[1], event->midi[2], noteon_result, voices_before, voices_after,
                        event->position);
            }
            break;
        }
    ...
    if (shot_rendered && shotprobe_enabled())
    {
        DWORD i;
        int peak = 0;
        for (i = 0; i < render_length * 2; ++i)
            peak = max(peak, abs(render_start[i]));
        ERR("SHOTPROBE block synth=%p frames=%lu peak=%d\n", This, render_length, peak);
    }
        ...
        struct instrument **out_instrument, struct region **out_region)
{
    ...
    *out_instrument = instrument;
    ...
    return found_region ? FLUID_OK : FLUID_FAILED;
}
...
static int synth_preset_noteon(fluid_preset_t *fluid_preset, fluid_synth_t *fluid_synth, int chan, int key, int vel)
{
    ...
    if (synth->shotprobe_pending && shotprobe_enabled())
        ERR("SHOTPROBE lookup synth=%p group=%lu channel=%d patch=%#x instrument=%p region=%p wave=%p samples=%u result=%d\n",
                synth, synth->shotprobe_group, chan, patch, instrument, region,
                region ? region->wave : NULL, region && region->wave ? region->wave->sample_count : 0, result);

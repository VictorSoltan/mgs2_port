#!/bin/sh
# FINALPLAY16-equivalent timing control. The only production substitution is
# the bounded memory-only D3D9 PRESENT counter required by the external harness.
set -eu

HERE=$(cd "$(dirname "$0")" && pwd)
LOGDIR="$HERE/logs/dxvk-sarek-production-counter"
mkdir -p "$LOGDIR"

D3D8=d3d8_dxvk_sarek_1.11.1_mali_wsiinit3.dll
D3D9=d3d9_dxvk_sarek_1.11.1_mali_count1.dll
BOX86=box86-fp16-dxvk
...
verify_one() {
    ...
    if [ "$got" != "$want" ]; then
        echo "MGS2 production counter control: $file is ${got:-missing}, expected $want" >&2
        exit 1
    fi
}
...
verify_one 104c79bcc15bd9f36a78e95ce657be35de2f5bae6e6f63008831f3164bda1cde "$BOX86"

export MGS2_RESEARCH_RUN=dxvk-sarek-production-counter
...
export MGS2_IDENTITY_MANIFEST=FINALPLAY16_DXVK_COUNTER_CONTROL.manifest
...
echo "MGS2 DXVK: FINALPLAY16-equivalent counter control, state_cache=0" >&2

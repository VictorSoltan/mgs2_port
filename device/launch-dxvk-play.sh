#!/bin/bash
# Research copy of the FINALPLAY15 runtime launcher for the DXVK-Sarek arm.
# The production launch-play.sh remains byte-identical to the promoted release.
...
# Research-only native renderer pair. MGS2 ships an app-local d3d8.dll, which
...
start_gptokeyb() {
    ...
    if [ -n "${GPTOKEYB:-}" ]; then
        # PortMaster supplies the device/OS-specific kill-mode option.

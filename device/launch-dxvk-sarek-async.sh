#!/bin/sh
# Research-only DXVK-Sarek async arm. Missing graphics pipelines may skip a
# draw until a background compiler finishes, so performance and visual
# correctness must be evaluated separately before any promotion.
set -eu

HERE=$(cd "$(dirname "$0")" && pwd)
LOGDIR="$HERE/logs/dxvk-sarek-async"
mkdir -p "$LOGDIR"

D3D8=d3d8_dxvk_sarek_1.11.0_async_mali_wsi1.dll
D3D9=d3d9_dxvk_sarek_1.11.0_async_mali_count1.dll
...
verify_one() {
    ...
    if [ "$got" != "$want" ]; then
        echo "MGS2 DXVK async: $file is ${got:-missing}, expected $want" >&2
        exit 1
    fi
}

verify_one 2b811cf2bf86309c44ccf9af0161d366289b1e1554e275010f96d26e40bb4131 "$D3D8"
verify_one a15e8a3d4590cb04cb7752bdeada1c974c6c5254295021b09a2720c26242d111 "$D3D9"
verify_one 83f9349c6dc26f8f769e714a5ed57c4d76f3a523161ead31f75e52ccc1da7fba "$BOX86"

export MGS2_RESEARCH_RUN=dxvk-sarek-async-d3d8
...
export MGS2_IDENTITY_MANIFEST=FINALPLAY16_DXVK_ASYNC_HITCH.manifest
...
export DXVK_STATE_CACHE_PATH="${MGS2_DXVK_STATE_CACHE_PATH:-$HERE/cache/dxvk-state-async}"
...
echo "MGS2 DXVK async: RESEARCH arm, async=$DXVK_ASYNC threshold=$ASYNC_DRAW_CALL_THRESHOLD state_cache=$DXVK_STATE_CACHE" >&2
echo "MGS2 DXVK async: d3d8=$D3D8 d3d9=$D3D9 box86=$BOX86" >&2

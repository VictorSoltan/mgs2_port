#!/bin/sh
# Research-only FINALPLAY16 + fused DXT5 surface-row differential verifier.
set -eu

HERE=$(cd "$(dirname "$0")" && pwd)
LOGDIR="$HERE/logs/dxvk-sarek-dxt-surface"
mkdir -p "$LOGDIR"
...
verify_one() {
    ...
    if [ "$got" != "$want" ]; then
        echo "MGS2 native DXT surface: $file is ${got:-missing}, expected $want" >&2
        exit 1
    fi
}
...
export MGS2_RESEARCH_RUN=dxvk-sarek-native-dxt-surface-verify
...
export MGS2_IDENTITY_MANIFEST=FINALPLAY16_DXVK_DXT_SURFACE_VERIFY.manifest
...
echo "MGS2 DXVK: RESEARCH native-DXT-surface mode=$MGS2_BOX86_NATIVE_DXT_SURFACE" >&2

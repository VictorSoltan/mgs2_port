...
thermal_guard() {
    ...
    while kill -0 "$WINE_PID" 2>/dev/null; do
        ...
        if [ "$temp" -ge 88000 ]; then
            # One bounded cold-path record makes a deliberate thermal SIGTERM
            # distinguishable from a Wine/game crash after teardown.
...
printf '%s launcher=finalplay15-wined3d pid=%s status=%s reason=%s\n' \

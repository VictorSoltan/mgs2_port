#!/bin/sh
# Build the exact FINALPLAY22 state-owned wpatch view from a legal MGS2 copy.
#
# This retains the water/IPU split and lighting cleanup, makes the fixed-function
# wpatch path select COUNT2 before uploading its UV matrix, and keeps that path
# selected if device creation falls back to software VP. The installed game
# image is never changed.
set -eu

INPUT=${1:?usage: patch-mgs2-wpatch-finalplay22.sh INPUT OUTPUT}
OUTPUT=${2:?usage: patch-mgs2-wpatch-finalplay22.sh INPUT OUTPUT}
...
case "$OUTPUT" in
    /tmp/mgs2-wpatch-finalplay22.*) ;;
    *)
        echo "MGS2: refusing output outside the private /tmp FINALPLAY22 name" >&2
        exit 1
        ;;
esac
if [ -L "$OUTPUT" ]; then
...
if [ "$got" != "$ORIGINAL_SHA256" ]; then
    echo "MGS2: game EXE is ${got:-missing}, FINALPLAY22 expects $ORIGINAL_SHA256" >&2
    exit 1
fi
...
echo "MGS2: generated exact FINALPLAY22 game EXE $got" >&2

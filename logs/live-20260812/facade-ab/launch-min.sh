#!/bin/bash
# MGS2 Substance RG353VS -- MINIMAL stack, laboratory only, NOT production.
# Splits the patch stack in half to locate the intermittent freeze. Keeps the GLES
# foundation (win32u bridge, opengl32 version parse, present path, box86) and the
# audio work; drops the whole renderer optimisation chain: patches 12, 18-27.
# Expect a large fps regression -- that is the price, not a defect.
...
mount_bind "$GAMEDIR/wined3d_release3.dll" /usr/lib/wine/i386-windows/wined3d.dll || exit 1
# user32_peek1.dll (patch 07) deliberately NOT mounted: stock user32.
mount_bind "$GAMEDIR/d3d8_mgs2fast1.dll" /usr/lib/wine/i386-windows/d3d8.dll || exit 1

#!/bin/bash
# MGS2 Substance RG353VS -- AUDIT: which fast paths actually do work. MIN30 base: the smallest stack measured at 30 fps.
...
# AUDIT run: the only two builds that still carry telemetry. Patch 25 deleted the
# counters from the FINAL DLLs, so "is this fast path earning anything" can only
# be answered on the pre-25 pair. Non-FINAL builds read the env knobs.
export MGS2_GLES_FBO_READBACK=1
export MGS2_SKIP_CODEC_BLACK_QUAD=0
export MGS2_BATCH="${MGS2_BATCH:-1}"
export MGS2_BATCH_STATS=1
export MGS2_D3D8_STATS=1
export MGS2_D3D8_VISIBILITY_CULL="${MGS2_D3D8_VISIBILITY_CULL:-1}"
export MGS2_D3D8_VISIBILITY_CULL_STATS=1
...
mount_bind "$GAMEDIR/wined3d_batch16_setcache.dll" /usr/lib/wine/i386-windows/wined3d.dll || exit 1
...
mount_bind "$GAMEDIR/d3d8_producer_batch20_visibilitycull.dll" /usr/lib/wine/i386-windows/d3d8.dll || exit 1
